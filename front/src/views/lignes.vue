  ...
  <div class="flex flex-col min-h-full w-full pb-safe">
    ...
    <header class="sticky top-0 z-50 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md px-6 pt-6 pb-2 border-b border-transparent transition-colors duration-300">
      ...
      <div class="flex justify-between items-center mb-5">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Le Réseau</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Explorez toutes les lignes</p>
        </div>
        <ThemeToggle />
      </div>
      ...
      <div class="relative mb-2">
        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
          <span class="material-icons-round text-primary/70">search</span>
        </div>
        <input
          v-model="searchTerm"
          type="text"
      ...
      <template v-else-if="groupedLignes.length > 0">
        <section v-for="group in groupedLignes" :key="group.title">
          ...
          <div class="flex items-center gap-2 mb-3 px-1 pt-2">
            <span :class="['material-icons-round text-lg', group.iconColor]">{{ group.icon }}</span>
            <h2 class="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ group.title }}</h2>
          </div>
          ...
          <div class="space-y-3">
            <router-link
              ...
              class="bg-surface-light dark:bg-surface-dark rounded-2xl p-4 shadow-soft border border-gray-100 dark:border-gray-800 flex items-center justify-between group active:scale-[0.98] transition-transform duration-150 cursor-pointer block"
            >
              <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col">
                  <span class="font-bold text-gray-900 dark:text-gray-100 text-[15px]">
              ...
              <span class="material-icons-round text-gray-400 dark:text-gray-600 group-hover:text-primary transition-colors">chevron_right</span>
            </router-link>
          </div>
        </section>
      </template>
