    ...
    <header class="sticky top-0 z-50 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md px-4 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 border-b border-gray-100 dark:border-gray-800 transition-colors duration-300">
      <div class="flex items-center gap-3 sm:gap-4">
        ...
        <div class="flex-grow min-w-0">
          ...
          <p v-if="details" class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
    ...
    <main class="flex-grow px-4 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
      ...
      <template v-else-if="details">
        ...
        <div class="text-center mb-1 sm:mb-2">
          ...
          <p v-if="details.constructeur && details.modele" class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
        ...
        <div class="grid grid-cols-2 gap-2.5 sm:gap-3">
          ...
          <div :class="['p-3 sm:p-4 rounded-xl sm:rounded-2xl flex flex-col items-center gap-1.5 sm:gap-2', energieConfig.bgColor]">
            ...
            <span :class="['text-xs sm:text-sm font-semibold', energieConfig.color]">{{ energieConfig.label }}</span>
          </div>
          ...
          <div class="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-surface-light dark:bg-surface-dark flex flex-col items-center gap-1.5 sm:gap-2 shadow-soft">
            ...
            <span :class="['text-xs sm:text-sm font-semibold', accessibiliteConfig.color]">{{ accessibiliteConfig.label }}</span>
          </div>
          ...
          <div class="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-surface-light dark:bg-surface-dark flex flex-col items-center gap-1.5 sm:gap-2 shadow-soft">
            ...
            <span :class="['text-xs sm:text-sm font-semibold', climConfig.color]">{{ climConfig.label }}</span>
          </div>
        ...
        <div class="bg-surface-light dark:bg-surface-dark rounded-xl sm:rounded-2xl shadow-soft overflow-hidden">
          <div class="px-4 sm:px-5 py-2.5 sm:py-3 border-b border-gray-100 dark:border-gray-800">
            <h3 class="font-semibold text-gray-900 dark:text-white text-xs sm:text-sm uppercase tracking-wide">Détails</h3>
          </div>
          <div class="divide-y divide-gray-100 dark:divide-gray-800">
            <!-- Numéro de parc -->
            <div class="flex justify-between items-center px-4 sm:px-5 py-3 sm:py-3.5">
              <span class="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Numéro de parc</span>
              <span class="font-medium text-sm sm:text-base text-gray-900 dark:text-white">{{ details.numParc }}</span>
            </div>
            
            <!-- Constructeur -->
            <div v-if="details.constructeur" class="flex justify-between items-center px-4 sm:px-5 py-3 sm:py-3.5">
              <span class="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Constructeur</span>
              <span class="font-medium text-sm sm:text-base text-gray-900 dark:text-white">{{ details.constructeur }}</span>
            </div>
            
            <!-- Modèle -->
            <div v-if="details.modele" class="flex justify-between items-center px-4 sm:px-5 py-3 sm:py-3.5">
              <span class="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Modèle</span>
              <span class="font-medium text-sm sm:text-base text-gray-900 dark:text-white">{{ details.modele }}</span>
            </div>
            
            <!-- Longueur -->
            <div v-if="details.longueurVehicule" class="flex justify-between items-center px-4 sm:px-5 py-3 sm:py-3.5">
              <span class="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Longueur</span>
              <span class="font-medium text-sm sm:text-base text-gray-900 dark:text-white">{{ details.longueurVehicule }}</span>
            </div>
            
            <!-- Date mise en service -->
            <div v-if="details.dateMiseService" class="flex justify-between items-center px-4 sm:px-5 py-3 sm:py-3.5">
              <span class="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Mise en service</span>
              <span class="font-medium text-sm sm:text-base text-gray-900 dark:text-white">{{ details.dateMiseService }}</span>
            </div>
          </div>
        </div>
      ...
      <div v-else class="text-center py-12 sm:py-16">
        ...
        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">Aucune information disponible pour ce véhicule</p>
      </div>
    </main>
  </div>
</template>
